...
class HomeViewModel: ObservableObject {
	...
	func addSubscribers() {
		...
		$allCoins
			...
			.sink { [weak self] returnedCoins in
				...
				self.portfolioCoins = self.sertPortfolioCoinsIfNeeded(coins: returnedCoins)
			}
			.store(in: &cancellables)
	...
	private func filerAndSortCoins(text:String, coins:[CoinModel], sort:SortOption) -> [CoinModel]  {
		...
		sortCoins(sort: sort, coins: &updatedCoins) // & = out C#
	...
	private func sertPortfolioCoinsIfNeeded(coins: [CoinModel]) -> [CoinModel] {
		switch sortOption {
			case .holdings:
	...
	private func mapAllCoinsToPotrfolioCoins(allCoins: [CoinModel], portfolioEntities: [PortfolioEntity]) -> [CoinModel] {
